...
def overlay_plots(sensor_of_interest, files, key, values):
    ...
    # sensor_of_interest is the single tested sensor in SQC, important to evaluate whether its parameters are within the expected range
    ...
    for file in files:
       ...
       if (y.abs()).max() > (100*y.abs()).median() and "Str" in file:
         
          plt.yscale('log')
          
       
       if sensor_of_interest in file:
           flag = sqc.evaluate_results(y, values, key, '2-S' if '2-S' in file else 'PSS')
...
def make_pdf_with_plots(sensor, files, measurement_config, pdf_name):

  # produces the pdfs with the plots
  
  dictionary_with_flags = {}
  ...
  with PdfPages('{}.pdf'.format(pdf_name)) as pdf:
      for key,values in measurement_config.items():
         ...
         fig = plt.figure()
         flag = overlay_plots(sensor, files, key, values) 
         pdf.savefig(fig)
         dictionary_with_flags[key] = flag


  return dictionary_with_flags
...
def main():
    ...
    if len(IVCV_files)>=1:
        dictionary_witf_flags = make_pdf_with_plots(args.sensor, IVCV_files, IV_config, "IVCV")
        
    if len(Stripscan_files)>=1:
        dictionary_with_flags = make_pdf_with_plots(args.sensor, Stripscan_files, Stripscan_config, "Stripscan")
